...
class ScraperILJohnsonSuperior(ScraperBase):
    ...
    def get_case_information(self, case_url):
        ...
        if table:
            ...
            information_key = ''
            object_key = []
            keys_for_object = ['Litigant Information',
                ...
                if row.has_key('class'):
                    ...
                    if information_key == '':
                        if row['class'][0] == 'case_level_2':
                            case_information = {
                                **case_information, **(self.parse_table_row(row))}
                    elif information_key in keys_for_object:
                        if information_key not in case_information:
                            case_information[information_key] = {}
                        case_information[information_key] = {
                            **case_information[information_key], **(self.parse_table_row(row))}
    ...
    def parse_search_results(self, soup, input_dob):
        ...
        for case_row in case_rows:
            if not case_row.has_key('class') or case_row['class'][0] == 'field_4':
                continue
    ...
    def search_in_johnson_il(self, first_name, last_name, dob):
        ...
        try:
            r = self.GLOBAL_SESSION.get(self.SEARCH_RESULT_PAGE_URL.format(
        ...
        if last_page_link:
            ...
            for page in range(0, last_page + 1):
                try:
                    r = self.GLOBAL_SESSION.get(self.SEARCH_RESULT_PAGE_URL.format(
                        self.COURT_ID, last_name + '%2C+' + first_name, page))
                except requests.ConnectionError as e:
                    print("Connection failure : " + str(e))
                    print("Verification with InsightFinder credentials Failed")
                    return {'error': str(e)}
