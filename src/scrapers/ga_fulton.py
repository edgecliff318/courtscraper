...
from src.scrapers.base import NameNormalizer, ScraperBase, get_recaptcha_answer
...
class ScraperGAFultonSuperior(ScraperBase):
        ...
        if disposition_information_body:
            ...
            for item in items.findAll("div", class_="row-buff"):
                ...
                if item.find("div", class_="tyler-toggle-container").find("p"):
                    if (
                        item.find("div", class_="tyler-toggle-container")
                        .find("p")
                        .find("span", class_="text-muted")
                    ):
                        position = (
                            item.find("div", class_="tyler-toggle-container")
                            .find("p")
                ...
                dispositionItem = {
                    "date": item.find("div", class_="tyler-toggle-controller")
                    .text.replace("\n", "")
                    .replace("   ", " ")
                    .strip(),
                    "position": position,
    ...
    def get_charge_information(self, soup):
        ...
        if charge_information_body:
            charges = ""
            if (
                len(charge_information_body.findAll("div", class_="col-md-12"))
                > 1
            ):
                charges = (
                    charge_information_body.findAll("div", class_="col-md-12")[
                        1
                    ]
                    .text.replace("Charges", "")
                    .strip()
                )
            charge_information = {"name": charges, "charges": []}
        ...
        if events_information_body:
            ...
            for item in items.findAll("li", class_="list-group-item"):
                if item.find("div", class_="tyler-toggle-controller"):
                    event = {
                        "Event": item.find(
                            "div", class_="tyler-toggle-controller"
                        )
                        .text.replace("\n", "")
                        .replace("   ", " ")
                        .strip()
                    ...
                    options = item.find(
                        "div", class_="tyler-toggle-container"
                    ).findAll("p")
    ...
    def get_search_result(self, html):
        ...
        """
        if (
            len(html.split(',"data":')) > 1
            and len(
                html.split(',"data":')[1].split(
                    """},"detailTemplate":kendo.template($('#PartyCardTemplate')"""
                )
            )
            > 0
            and "Data"
    ...
    def search_in_fulton_ga(self, first_name, last_name, dob):
        ...
        self.HEADERS = {
            "Cookie": "BNI_reco_cookie02684616={}; BNI_reco_cookie02684616={}; BNI_reco_cookie02684616={}; BNES_ASP.NET_SessionId={}; BNES_SameSite=0KCds494YizK+0ffxndFt9/4dsqCwi4btM9Ei3JM+NJDD2zFRISf/DsPxbfMdcjx5/JCk+o6Gg8=; BNES_SmartSearchCriteria={}".format(
                cookies["BNI_reco_cookie02684616"],
                cookies["BNI_reco_cookie02684616"],
                cookies["BNI_reco_cookie02684616"],
                cookies["BNES_ASP.NET_SessionId"],
                cookies["BNES_SmartSearchCriteria"],
            )
        }
        ...
        for mInd, match in enumerate(matches):
            for cInd, case in enumerate(match["CaseResults"]):
                ...
                matches[mInd]["CaseResults"][cInd][
                    "CaseDetail"
                ] = self.get_case_detail(case_detail_url, case_id)
...
if __name__ == "__main__":
    ...
    print(
        json.dumps(
            ScraperGAFultonSuperior().scrape(
                search_parameters={
                    "firstName": "Stuart",
                    "lastName": "Baker",
                    "dob": "",
