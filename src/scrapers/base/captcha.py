...
def get_recaptcha_answer(site_key, pageurl):
    try:
        captcha_id = session.post(
            "{}in.php?key={}&method=userrecaptcha&googlekey={}&pageurl={}".format(
                BASE_URL, CAPTCHA_KEY, site_key, pageurl
            )
        ).text.replace("OK|", "")
    ...
    try:
        recaptcha_answer = session.get(
            "{}res.php?key={}&action=get&id={}".format(
                BASE_URL, CAPTCHA_KEY, captcha_id
            )
        ).text
    except requests.ConnectionError as e:
        print("Connection failure : " + str(e))
