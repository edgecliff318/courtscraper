...
class ScraperMIWayneSuperior(ScraperBase):
    ...
    SEARCH_RESULT_URL = (
        "https://dapps.36thdistrictcourt.org/ROAWEBINQ/ROASched.aspx"
    )
    CASE_DETAIL_URL = (
        "https://dapps.36thdistrictcourt.org/ROAWEBINQ/ROACase.aspx"
    )
    ...
    def get_case_detail(self, case_number, page_number, last_name, first_name):
        ...
        try:
            r = self.GLOBAL_SESSION.post(
                SPLASH_URL,
                auth=(SPLASH_USERNAME, SPLASH_PASSWORD),
                json={
                    "url": self.SEARCH_URL,
            ...
            if "html" in json.loads(r.text):
                return self.parse_case_detail(
                    BeautifulSoup(
                        json.loads(r.text)["html"], features="html.parser"
                    )
                )
            else:
                return {}
    ...
    def parse_case_detail(self, soup):
        ...
        keys = [
            ...
            ["DOB:", "SEX:", "RACE:", "CDL:"],
            ["VEHICLE\xa0TYPE:", "VPN:"],
            ["VEH\xa0YR:", "VEH\xa0MAKE:", "PAPER\xa0PLATE:"],
            ["OFFICER:", "DEPT:"],
            ["PROSECUTOR:"],
        ]
        offenseKeys = [
            ["CNT:", "C/M/F:"],
            ["ARRAIGNMENT\xa0DATE:", "PLEA:", "PLEA\xa0DATE:"],
            ["FINDINGS:", "DISPOSITION\xa0DATE:"],
            ["SENTENCING\xa0DATE:"],
        ]
        status = ""
        count = 0
        for row in table.findAll("tr"):
            ...
            for key in keys:
                if key[0] in row_content:
                    if len(key) == 1:
                        case_detail[
                            key[0].replace(":", "").replace("\xa0", " ")
                        ] = (
                            row_content.split(key[0])[1]
                            .replace("|", "")
                            .replace("\xa0\xa0", " ")
                            .replace("\xa0", " ")
                            .strip()
            ...
            if status == "Offense" and count == 1:
                case_detail["Offenses"][len(case_detail["Offenses"]) - 1][
                    "Offense"
                ] = (
                    row_content.replace("\xa0\xa0", " ")
                    .replace("\xa0", " ")
                    .replace("|", "")
                    .strip()
                )
            ...
            if status == "DEFENSE ATTORNEY ADDRESS":
                if count == 1:
                    ...
                    case_detail["DEFENSE ATTORNEY ADDRESS"] = row_content[
                        0:start
                    ].strip()
                    case_detail["BAR NO."] = (
                        row_content[start : len(row_content) - 1]
                        .replace("\xa0\xa0", " ")
                        .replace("\xa0", "")
                        .strip()
                    )
                    ...
                    start = row_content.find("\xa0\xa0")
                    case_detail["DEFENSE ATTORNEY ADDRESS"] = (
                        case_detail["DEFENSE ATTORNEY ADDRESS"]
                        + " "
                        + row_content[0:start]
                        .replace("\xa0\xa0", " ")
    ...
    def convertDobToStr(self, dob):
        ...
        return str(
            int(datetime.strftime(dobString, "%m"))
        ) + datetime.strftime(dobString, "/%d/%Y")
