...
class WestVirginiaScraper(ScraperBase):
    ...
    async def search_by_name(self, search_parameter):
        ...
        data = {
            ...
            "selectedCourts": [],
            "searchString": [f"{search_parameter}"],
            "searchBy": "N",
        }
        res = requests.post(url, json=data, cookies=self.req_cookies)
        res_dict = res.json()
        search_result = res_dict["context"]["entity"]["payload"][
            "searchResults"
        ]
    ...
    def get_case_detail(self, search_result):
        ...
        court_id = (
            caseCourt["fipsCode"] + caseCourt["courtCategoryCode"]["value"]
        )

        caseParticipant = detail_data["context"]["entity"]["payload"][
            "caseParticipant"
        ]
        for participant in caseParticipant:
            if participant["participantCode"] == "DEF":
                ...
                middle_name = contact_info["personName"].get(
                    "personMiddleName"
                )
                address_city = contact_info["primaryAddress"].get(
                    "locationCityName"
                )
                address_zip = contact_info["primaryAddress"].get(
                    "locationState"
                )
                address_state_code = contact_info["primaryAddress"].get(
                    "locationPostalCode"
                )
                gender = participant["personalDetails"]["gender"]
                birth_date = participant["personalDetails"].get(
                    "maskedBirthDate"
                )
        case_dict = {
            "case_id": case_id,
