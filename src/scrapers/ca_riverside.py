...
class ScraperCARiversideSuperior(ScraperBase):
                            ...
                            defendents.append(
                                {
                                    ...
                                    .split("  ")[0]
                                    .strip(),
                                    "next_court_date": cells[2]
                                    .split("  ")[0]
                                    .strip(),
                                    "status": cells[3].split("  ")[0].strip(),
                                    "agency_number": cells[4]
                                    .split("  ")[0]
                                    .strip(),
                                    "arrest_date": cells[5]
                                    .split("  ")[0]
                                    .strip(),
                                    "count1_charge": cells[6]
                                    .split("  ")[0]
                                    .strip(),
                                    "violation_date": cells[7]
                                    .split("  ")[0]
                                    .strip(),
                                }
                            )
    ...
    def parse_search_results(self, soup):
        ...
        """
        cases = []
        search_matches_container = soup.find(
            "div", {"id": "divNameSearchMatches"}
        )
        if not search_matches_container:
            return []
    ...
    def search_in_riverside_ca(self, first_name, last_name, dob):
        ...
        try:
            r = self.GLOBAL_SESSION.post(
                self.SEARCH_CASE_URL,
                {
                    ...
                    "ReturnToURL": "http%3A%2F%2Fpublic%2Daccess%2Eriverside%2Ecourts%2Eca%2Egov%2FOpenAccess%2FCriminalSearchByNameDOB%2Easp%3FtxtLastName%3D{}%26txtFirstName%3D{}%26txtMiddleName%3D%26ShowMatches%3DFalse%26MatchNumber%3D%26optPerBusInd%3DPER%26hidRivInd%3DIND%26hidDOB%3D{}".format(
                        last_name, first_name, dob_string
                    ),
                    "MatchNumber": "1",
                    "hidPerBusInd": "PER",
        ...
        for case in cases:
            url = "{}?CourtCode=C&RivInd=IND&CaseNumber={}&DefNbr=175830&DefSeq=1&otnmseq=0".format(
                self.CASE_DETAIL_URL, case["case_number"]
            )
            try:
...
if __name__ == "__main__":
    print(
        json.dumps(
            ScraperCARiversideSuperior().scrape(
                search_parameters={
                    "firstName": "Joyce",
                    "lastName": "Washington",
                    "dob": "1/23/1961",
