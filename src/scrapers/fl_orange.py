...
from src.scrapers.base import NameNormalizer, TextNormalizer, ScraperBase, InitializedSession, get_recaptcha_answer, SPLASH_USERNAME, SPLASH_PASSWORD, SPLASH_URL
import re
import requests
...
class ScraperFLOrangeSuperior(ScraperBase):
    ...
    def parse_case_detail(self, soup):
        ...
        if case_header_table:
            for row in case_header_table.findAll('div', class_='row'):
                key = row.find(
                    'div', class_='col-md-5').text.strip().replace(':', '')
                case_detail[key] = row.find(
                    'div', class_='col-md-7').text.strip()
        ...
        charge_details_table = soup.find(
            'div', {'id': 'chargeDetailsCollapse'})
    ...
    def search_in_orange_fl(self, first_name, last_name, dob):
        ...
        try:
            ...
            cases = self.parse_search_results(BeautifulSoup(
