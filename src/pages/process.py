...
def page(data: dict = None, filemanager=True):
    ...
    image_filename = settings.DATA_PATH.joinpath(
        data.get("image").name
    ...
    parser_card = dbc.Row(
        [
            dbc.Col(
                dbc.Card(
                    [dbc.CardHeader(html.H5("Form")), dbc.CardBody(form_card)],
                    class_name="mb-2",
                )
            ),
            dbc.Col(
                dbc.Card(
                    [dbc.CardHeader(html.H5("Scan")), dbc.CardBody(image)],
                    class_name="mb-2",
                )
            ),
        ]
    ...
    if file_manager_card is not None:
        results.append(dbc.Col(file_manager_card, width=12))
    results.append(dbc.Col(parser_card, width=12))
