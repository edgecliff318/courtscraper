...
def render_content_persona_details_selector(pathname):
    ...
    options = [{"label": c.get("label"), "value": c.get("value")} for c in courts]
    return options
        ...
        for case_type in ("Traffic%2FMunicipal", "Infraction"):
            for court_code in court_code_list:
                config_loader = ConfigLoader(
                    path=os.path.join(settings.CONFIG_PATH, "config.json")
                )
...
def render_leads(search, court_code_list, start_date, end_date, interaction):
    ...
    if trigger_id == "leads-button":
        lead_loader = LeadsLoader(path=os.path.join(settings.CONFIG_PATH, "leads.json"))
        data = lead_loader.load()
                    ...
                    html.Tbody(
                        [
                            html.Tr(
                                [
                                    html.Td(
                                        i.get("date"), style={"font-weight": "700"}
                                    ),
...
def render_case_details(pathname):
    if "/leads/single" in pathname:
        ...
        if error:
            ...
            if results.get("ticket") is not None:
                ticket = content.process.page(results.get("ticket"), filemanager=False)
            else:
                logger.error(
                    f"No ticket found for case {case_id}, " f"received {results}"
                )
                ticket = []
            ...
            first_name, last_name, link = get_verified_link(name, year_of_birth)

            case_details_stored = {"first_name": first_name}
            ...
            return (
                [
                    ...
                    dbc.Col(
                        get_table_data(
                            "Charges",
                            results.get("charges", {}).get("Charge/Judgment", {}),
                        ),
                        width=6,
                    ),
                    *ticket,
