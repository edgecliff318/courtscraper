...
case_detail = BeautifulSoup(
    open("mocks/ca_san_diego/case_detail.html", "r+", encoding="utf-8").read(),
    features="html.parser",
)
search_results = BeautifulSoup(
    open(
        "mocks/ca_san_diego/search_results.html", "r+", encoding="utf-8"
    ).read(),
    features="html.parser",
)
...
def test_search_in_san_diego_ca_for_lambda():
    ...
    actual = ScraperCASanDiegoSuperior().scrape(
        search_parameters={
            "firstName": "Zeus",
            "lastName": "Robles",
            "dob": "07/03/1986",
