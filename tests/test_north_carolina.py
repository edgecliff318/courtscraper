...
offender_detail = BeautifulSoup(
    open(
        "mocks/north_carolina/offender_detail.html", "r+", encoding="utf-8"
    ).read(),
    features="html.parser",
)
search_results = BeautifulSoup(
    open(
        "mocks/north_carolina/search_results.html", "r+", encoding="utf-8"
    ).read(),
    features="html.parser",
)
expected_result = json.loads(
    open(
        "mocks/north_carolina/north_carolina.json", "r+", encoding="utf-8"
    ).read()
)
...
def test_search_in_travis_tx_for_lambda():
    ...
    actual = ScraperNCSuperior().scrape(
        search_parameters={
            "firstName": "Adam",
            "lastName": "Smith",
            "dob": "12/19/1967",
