...
case_detail = json.loads(
    open(
        "mocks/ca_santa_clara/case_detail.json", "r+", encoding="utf-8"
    ).read()
)
search_results = open(
    "mocks/ca_santa_clara/search_results.json", "r+", encoding="utf-8"
).read()
expected_result = json.loads(
    open(
        "mocks/ca_santa_clara/ca_santa_clara.json", "r+", encoding="utf-8"
    ).read()
)
...
def test_get_case_detail_for_site():
    """Tests scraper parsing functionality."""
    actual = ScraperCASantaClaraSuperior().get_case_detail(
        json.loads(search_results)["data"][0]["caseId"]
    )["case_detail"]
...
def test_search_in_santa_clara_ca_for_lambda():
    ...
    actual = ScraperCASantaClaraSuperior().scrape(
        search_parameters={
            "firstName": "Stuart",
            "lastName": "Baker",
            "dob": "10/27/1963",
