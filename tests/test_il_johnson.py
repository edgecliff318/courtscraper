...
search_results = BeautifulSoup(
    open(
        "mocks/il_johnson/search_results.html", "r+", encoding="utf-8"
    ).read(),
...
def test_parse_search_results_for_scraper():
    ...
    actual = ScraperILJohnsonSuperior().parse_search_results(
        search_results, expected_result[0]["dob"]
    )
...
def test_get_case_detail():
    ...
    for i in range(0, len(expected_result)):
        actual = ScraperILJohnsonSuperior().get_case_detail(
            expected_result[i]["case_url"]
        )
...
def test_get_case_information():
    """Tests scraper parsing functionality."""
    for i in range(0, len(expected_result)):
        actual = ScraperILJohnsonSuperior().get_case_information(
            expected_result[i]["case_url"]
        )
...
def test_get_case_detail_from_table():
    """Tests scraper parsing functionality."""
    for i in range(0, len(expected_result)):
        actual = ScraperILJohnsonSuperior().get_case_detail_from_table(
            expected_result[i]["case_url"].replace(
                "case_information", "case_dispositions"
            )
        )
        ...
        actual = ScraperILJohnsonSuperior().get_case_detail_from_table(
            expected_result[i]["case_url"].replace(
                "case_information", "case_payment_history"
            )
        )
        ...
        actual = ScraperILJohnsonSuperior().get_case_detail_from_table(
            expected_result[i]["case_url"].replace(
                "case_information", "case_fines_fees"
            )
        )
...
def test_search_in_travis_tx_for_lambda():
    ...
    actual = ScraperILJohnsonSuperior().scrape(
        search_parameters={
            "firstName": "Richard",
            "lastName": "Smith",
            "dob": "12/27/1978",
