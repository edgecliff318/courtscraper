...
case_detail = BeautifulSoup(
    open("mocks/az_maricopa/case_detail.html", "r+", encoding="utf-8").read(),
    features="html.parser",
)
search_results = BeautifulSoup(
    open(
        "mocks/az_maricopa/search_by_dob.html", "r+", encoding="utf-8"
    ).read(),
    features="html.parser",
)
...
def test_get_case_detail_for_scraper():
    """Tests scraper parsing functionality."""
    actual = ScraperAZMaricopaSuperior().get_case_detail(None, case_detail)[
        "case_detail"
    ]
